...
const Pins = ({ pin: { postedBy, image, _id, destination, save } }) => {
	...
	const savePin = (id) => {
		if (!alreadySaved) {
			// update the save data in the sanity db
			client
				.patch(id)
				.setIfMissing({ save: [] })
				.insert("after", "save[-1]", [
	...
	return (
		<div className="m-2">
			{/* Entire pin, clicking image will navigate to pinDetails */}
			<div
			...
			>
				{/* sanity's way to effectively fetch images. Optimized for set width.  */}
				<img
					className="rounded-lg w-full"
					alt="user-post"
					src={urlFor(image).width(250).url()}
				/>
				{/* Logic for when we hover a specific pin */}
				{postHovered && (
					<div
						className="absolute top-0 w-full h-full flex flex-col justify-between p-1 pr-2 pt-2 pb-2 z-50"
						style={{ height: "100%" }}
					>
						{/* flex col, This is the pin header div */}
						<div className="flex items-center justify-between">
							<div className="flex gap-2">
								{/* (1) Download image_url, but prevent further propagation of the current event. (stop from going to /pin-details*/}
								<a
						...
						{/* This is the pin footer div */}
						{/* (3) destination icon link or site user provides that host the image */}
